...
; Title: TIMER0 DELAY
; Description:
;       - Calculate a 60ms delay by using the TIMER0 module
;       - Each 60ms, toggle the value of a bit
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
MAIN:
        ...
        MOVLW   b'00000001'         	; TIMER = 16 BITS, INTERNAL TRANSITION, PRESCALER = 4
        MOVWF   T0CON               	;
...
; ---------------------- TIMER0 60ms SETUP ------------------------
;
; Formula to calculate the TMR0 value
;
;  Timer = 4 * Tosc * (8/16bits_max_value - TMR0value)*(Prescaler)
;  60ms = 4 * (1/16M) * (65536 - TMR0value) * (4)
;  TMR0value = 65536 - 60000
;  TRMR0value = 5536 -> 0x15A0
;  TMR0H = 0x15
;  TMR0L = 0xA0
; ----------------------------------------------------------------

SET_UP_TIMER0:
        MOVLW   0x15                ; LOAD THE 0X15A0 VALUE
        MOVWF   TMR0H               ; TO MAKE THE TIMER0
        MOVLW   0xA0                ; COUNT 5,536 TIMES
